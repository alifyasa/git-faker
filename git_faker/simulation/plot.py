...
def plot_func(
    ...
    if "x_step" in options and "y_step" in options:
        ...
        plt.xticks(
            np.arange(np.min(x), np.max(x) + 1, x_step),
            (np.arange(np.min(x), np.max(x) + 1, x_step) / x_step).astype(np.integer),
        )
        plt.yticks(
            np.arange(np.min(y), np.max(y) + 1, y_step),
            (np.arange(np.min(y), np.max(y) + 1, y_step) / y_step).astype(np.integer),
        )
